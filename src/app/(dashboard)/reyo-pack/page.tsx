  ...
  return (
    <section className={cn("rounded-2xl border p-5", cancelled ? "border-rose-400/40 bg-rose-400/10" : alreadyPacked ? "border-amber-400/40 bg-amber-400/10" : found ? "border-emerald-400/40 bg-emerald-400/10" : "border-rose-400/30 bg-rose-400/10")}>
      ...
      <div className="mt-5 grid grid-cols-2 gap-x-4 gap-y-4 text-sm">
        ...
        {result.carrier && <div><p className="text-[11px] uppercase tracking-wider text-zinc-500">Carrier</p><p className="mt-1 text-white">{result.carrier}</p></div>}
        {result.trackingNumber && <div><p className="text-[11px] uppercase tracking-wider text-zinc-500">Tracking</p><p className="mt-1 break-all font-mono text-white">{result.trackingNumber}</p></div>}
        {result.amazonPackageStatusDetail && <div className="col-span-2"><p className="text-[11px] uppercase tracking-wider text-zinc-500">Amazon package status</p><p className="mt-1 text-white">{result.amazonPackageStatusDetail}</p></div>}
      </div>
